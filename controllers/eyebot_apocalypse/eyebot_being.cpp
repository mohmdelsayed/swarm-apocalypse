...
void CEyeBotBeing::TakeOff()
{
   ...
   if (Distance(m_cTargetPos, m_pcPosSens->GetReading().Position) < POSITIONING_TOLERANCE)
   {
      ...
      CVector2 forces;
      if (m_HState == STATE_INFECTED){
         forces = m_sApocalypseParams.alpha_infected*HealthyFlockingVector() + m_sApocalypseParams.beta_infected*InfectedFlockingVector() + m_sApocalypseParams.gamma_infected*MedicFreeFlockingVector() + m_sApocalypseParams.delta_infected*MedicBusyFlockingVector();      } 
      if (m_HState == STATE_HEALTHY) {
         forces = m_sApocalypseParams.alpha_healthy*HealthyFlockingVector() + m_sApocalypseParams.beta_healthy*InfectedFlockingVector() + m_sApocalypseParams.gamma_healthy*MedicFreeFlockingVector() + m_sApocalypseParams.delta_healthy*MedicBusyFlockingVector();      }
      Flock(forces);
   }
}
...
void CEyeBotBeing::InfectedBehavior()
{
   ...
   if (InfectionTime < m_sApocalypseParams.InfectionStart && m_HState == STATE_INFECTED)
   {
      ...
      m_HState = STATE_INFECTED;
      m_pcRABAct->SetData(1, STATE_HEALTHY);
      CVector2 forces = m_sApocalypseParams.alpha_healthy*HealthyFlockingVector() + m_sApocalypseParams.beta_healthy*InfectedFlockingVector() + m_sApocalypseParams.gamma_healthy*MedicFreeFlockingVector() + m_sApocalypseParams.delta_healthy*MedicBusyFlockingVector();
      Flock(forces);
      InfectionTime += 1;
   }
   ...
   if (InfectionTime >= m_sApocalypseParams.InfectionStart && InfectionTime < m_sApocalypseParams.InfectionTerminal && m_HState == STATE_INFECTED)
   {
      ...
      m_HState = STATE_INFECTED;
      m_pcRABAct->SetData(1, STATE_INFECTED);
      CVector2 forces = m_sApocalypseParams.alpha_infected*HealthyFlockingVector() + m_sApocalypseParams.beta_infected*InfectedFlockingVector() + m_sApocalypseParams.gamma_infected*MedicFreeFlockingVector() + m_sApocalypseParams.delta_infected*MedicBusyFlockingVector();
      Flock(forces);
      InfectionTime += 1;
